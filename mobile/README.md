...
- **Pets** — a **🐾 Pets** hub on the Love tab (`PetsModal` → `PetShopModal` / `PetModal`; catalogue in `src/data/pets.ts`). Adopt from **18 options** across dogs, cats, birds, small pets, reptiles, fish, and even a horse — each with a price, yearly upkeep (country-scaled), and a lifespan. Every pet has its own **bond** and **mood**; once-per-year care actions: **play**, **give a treat**, **go for a walk** (dogs/horses — boosts your health too), **vet visit** (restores mood), **teach a trick** (60% chance), and **rehome**. Pets **age each year, cost upkeep**, and eventually **pass away of old age** (a sad log + happiness hit). Save format is v18.
